...
Unreal OCIO 2.2 `.ocioz` 패키지로 내보낼 수 있고, qualifier·power window·샷 스틸 비교·
평균 매칭이 이미지 시퀀스 경로에 연결되어 있습니다. 깨끗한 Windows PC와 Unreal 5.5–5.8
실기는 릴리스 게이트로 남아 있습니다.
...
기본 편집 단축키는 `Space` 재생/일시정지, `J/K/L` 역방향·정지·정방향 셔틀,
`J+K`/`K+L` 0.25배속 셔틀, `K`를 누른 채 `J/L` 한 프레임 이동,
`E` 선택 소스 끝에 추가, `W`/`,` 삽입, `D`/`.` 덮어쓰기, `Shift+R` 소스 교체,
`Home/End` 시작·끝, `Shift+Z` 전체 타임라인 맞춤, `←/→` 실제 프레임 이동, `↑/↓` 이전/다음
...
기존 샷 중간에 삽입하면 원본 샷은 양쪽에 보존되고, 덮어쓰기는 전체 길이를 유지합니다.
...
컬러 관리 노드에서 선택 클립의 창작 Look을 33³ 또는 65³ Cube로 내보내거나 Unreal OCIO 2.2
`.ocioz` 패키지(CLF, manifest, 차트, 설정 안내)를 만듭니다. Qualifier와 Power Window는
그레이드 스택에 추가되며 이미지 시퀀스는 float 기준으로 미리보기와 출력이 같습니다.
일반 영상의 공간 그레이드는 3D LUT로 구울 수 없어 출력 전에 차단합니다. 샷 스틸 캡처,
좌우 wipe / 상하 split, 평균 매칭은 같은 프로젝트에 저장됩니다.
