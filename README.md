...
시간 눈금 영역을 누르거나 드래그하면 재생 헤드와 미리보기를 탐색하고, 썸네일이 보이는
클립 본문을 누르면 선택합니다. 선택한 클립은 본문을 끌어 이동하고 가장자리를 끌어
트림합니다. Windows 탐색기의
...
추가하고 접거나 제거하는 구조입니다. 전체 트림은 모든 클립의 앞·뒤를 실제 원본
...
합성으로 최종 출력에 같은 위치로 번인됩니다. 스탬프는 영상 위에 상·하단 검은 바를
겹쳐 작업자, 영상 정보와 타임코드를 표시하므로 원본 화면 비율과 출력 크기는 바뀌지
않습니다. 불안정한 GES 텍스트 클립 대신 Qt 그래픽 레이어로 미리보기해 기본 영상 재생
파이프라인과 분리합니다.
