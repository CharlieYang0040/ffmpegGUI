...
사용하지 않습니다.
`Ctrl+Alt+T`를 누르거나 우측 자막 패널의 `+`를 누르면 재생 헤드 위치에 자막을
추가할 수 있습니다. 자막 바는 타임라인 아래쪽에 표시되며 내용과 길이를 수정하거나
삭제할 수 있습니다. 컷을 리플 삭제하거나 앞에 영상을 삽입하면 자막 시간도 함께
잘리거나 이동하고 Undo로 영상과 자막이 같이 복원됩니다. 자막은 미리보기에 바로
합성되고 최종 MP4에는 ASS 기반으로 번인됩니다.
자막 바 본체를 끌면 시간을 옮길 수 있고 양끝의 밝은 핸들을 끌면 시작·종료 구간을
직접 조절할 수 있습니다. 우측 패널에서 UTF-8 SRT를 가져오거나 현재 자막을 표준
밀리초 SRT로 내보낼 수 있으며, 여러 자막을 한 번에 가져와도 Undo 한 번으로 제거됩니다.
