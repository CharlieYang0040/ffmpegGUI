...
드래그 중에는 GES에 seek를 연속 요청하지 않고 캐시된 썸네일 프레임을 즉시 보여주며,
놓는 순간에만 실제 원본 프레임으로 정밀 탐색합니다. `Ctrl+휠`은 확대·축소, 일반 휠과
...
프로그램 모니터의 `스코프` 버튼을 열면 최종 표시 프레임 기준 Waveform, RGB Parade,
Vectorscope, Histogram을 전환해 볼 수 있습니다. 스코프는 기본적으로 닫혀 있으며 열었을
때만 최대 10fps로 최신 프레임을 분석합니다. D3D11 미리보기는 별도 GStreamer 분기로
재생을 방해하지 않고 화면에 전달된 같은 GPU 프레임을 비동기로 읽으며, CPU와 float
미리보기는 이미 생성된 픽셀을 재사용합니다.
