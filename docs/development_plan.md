...
- **재개 지점:** 렌더 계약(R1–R7)은 연결됐다. 병목은 컬러리스트 UI다. 오른쪽 인스펙터는
  280–380px 범용 노드 목록이라 Primary LGG·휠·곡선 캔버스·qualifier 뷰어·뷰어 위 윈도우가
  없고, 그레이드는 `이펙트` 카드 안의 SpinBox 나열이다. 다음은 **U1 컬러 작업 공간**이다.
  각 단계는 미리보기·출력·저장·undo/redo 회귀가 모두 연결된 경우에만 완료로 바꾼다.
...
- 컷 편집 레이아웃과 컬러 작업 레이아웃을 나눈다. 컬러 모드에서 인스펙터 폭 상한 380px를 푼다.
...
- 컬러 모드(또는 도킹 패널)를 추가한다. 프로그램 모니터 + 스코프 + **넓은 그레이드 패널**.
