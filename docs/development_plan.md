...
| U1–U5 컬러리스트 UI | 계획 | 엔진 파라미터는 연결됨. 인스펙터 280–380px, SpinBox, 휠·곡선 캔버스·뷰어 핸들 없음 | E1 레이아웃 뒤 U1 작업 공간, 이어서 U2 휠 |
| E1–E9 상용 NLE 편집 | 계획 | 마그네틱 단일 트랙, 에지 리플 트림, 삽입만, 프로그램 모니터 하나, 타임라인 높이 250px 고정 | E1 창 분할 → 스키머/JKL → 삽입·덮어쓰기 → 롤/슬립/슬라이드. 연결 클립·Roles는 범위 밖 |
...
- **재개 지점:** 렌더 계약(R1–R7)은 연결됐다. 병목은 작업 공간이다. 컬러는 280–380px
  범용 인스펙터의 SpinBox 나열이고, 편집은 고정 250px 타임라인·삽입 전용·프로그램 모니터
  하나다. 다음은 **E1 레이아웃**(편집/컬러 모드가 같은 창 분할을 씀)과 그 안의
  **U1 컬러 작업 공간**이다. 휠을 지금 이펙트 카드에 넣지 않는다. 각 단계는
