...
컴파일하고 FFmpeg `ass` 필터로 번인한다. 스탬프의 상·하단 바는 영상을 pad하거나 scale하지
않고 ASS 도형으로 화면 위에 합성하며, 작업자·영상 정보와 초 단위 타임코드 이벤트를 함께
만든다. 미리보기는 Qt 레이어가 담당한다. GES `GESTextOverlayClip`은 정확 seek 중
