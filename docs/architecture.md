...
드래그 scrub 중에는 flush seek를 게시한 직후 반환해 다음 포인터 위치가 이전 preroll을
기다리지 않게 하고, 마우스를 놓은 마지막 위치에서만 `ASYNC_DONE`까지 기다리는 정확
seek를 수행한다. 타임라인 눈금과 편집 피드백의 프레임 번호는 고정 FPS 환산이 아니라
...
HEVC, 안전할 때 stream-copy), 컨테이너(MP4, MKV, MOV)를 독립 축으로 유지한다.
