    ...
    def concat_media_files(
        ...
        try:
            ...
            try:
                ...
                # 프로세스 완료 대기
                returncode, stdout_text, stderr_output = communicate_process(process, cancel_token)

                # 오류 확인
                if returncode != 0:
                    stderr_output = stderr_output or "알 수 없는 오류"
    ...
    def concat_with_filter_complex(
        ...
        try:
            ...
            try:
                ...
                returncode, stdout_text, stderr_text = communicate_process(process, cancel_token)

                if returncode != 0:
                    error_msg = f"직접 구성한 filter_complex 명령 실패 (코드: {returncode}): {stderr_text}"
                    self.logger.error(error_msg)
                    raise Exception(error_msg)
                ...
                if returncode != 0:
                    error_msg = f"간단한 filter_complex 명령 실패 (코드: {returncode}): {stderr_text}"
                    self.logger.error(error_msg)
                    raise Exception(error_msg)
