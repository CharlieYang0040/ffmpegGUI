...
class TimelineComponent:
    ...
    def get_in_out_points(self) -> Tuple[int, int]:
        """Return the active In/Out range as 1-based inclusive frame numbers."""
    ...
    def _on_apply_trim_clicked(self):
        ...
        frame_count = 0
        if preview_area and getattr(preview_area, 'current_media_frame_count', 0) > 0:
            frame_count = preview_area.current_media_frame_count
        elif self.timeline_widget and self.timeline_widget.frame_count > 0:
            frame_count = self.timeline_widget.frame_count

        if frame_count <= 0:
            logger.warning("미디어 프레임 정보를 가져올 수 없습니다.")
            return

        # 프레임 단위로 트림 값 설정
        start_frame = max(0, in_point - 1) # 시작 프레임 (앞에서부터)

        # 뒤에서부터 트림할 프레임 수 계산
        # 종료 프레임이 마지막 프레임보다 작으면, 그 차이만큼 뒤에서 트림
        end_trim = max(0, frame_count - out_point)
    ...
    def seek_to_start(self):
        ...
        # 명령 생성 및 실행
        command = SeekFrameCommand(self.timeline_widget, current_frame, 1)
        from app.core.commands import command_manager
        command_manager.execute(command)
    ...
    def seek_to_end(self):
        ...
        # 명령 생성 및 실행
        command = SeekFrameCommand(self.timeline_widget, current_frame, last_frame)
        from app.core.commands import command_manager
        command_manager.execute(command)
