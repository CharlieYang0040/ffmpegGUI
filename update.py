...
class UpdateChecker(QObject):
    ...
    def __init__(self):
        ...
        logger.info("UpdateChecker 초기화됨")

    def check_for_updates(self):
        if self.updating:
            logger.info("이미 업데이트 확인 중")
            return
    ...
    def perform_update_check(self):
        try:
            ...
            logger.info(f"업데이트 확인 중 오류 발생: {str(e)}")
    ...
    def get_latest_version_info(self):
        ...
        logger.info(f"GitHub API 요청: {api_url}")
        ...
        logger.info(f"최신 버전: {latest_version}, 다운로드 URL: {download_url}")
    ...
    def is_newer_version(self, latest_version, current_version):
        ...
        logger.info(f"버전 비교: {latest_version} > {current_version} = {is_newer}")
        return is_newer
